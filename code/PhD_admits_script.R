...
admissions_scenario <- function(rejection_rate = 0.8, admission_attempts = 20, scenario_runs = 5000, create_pic = TRUE) {
...
outcome_tibble_creator <- function(outcomes = c("UC Irvine", "Brandeis", "UT Austin", "BU", "Normie"),
                                   probability = c(0.44, 0.1, 0.4, 0.02, 0.04)) {
...
outcomes_scenario <- function(outcomes_tibble = outcome_tibble_creator()) {
  # Theme setting - I chose the economist theme
  theme_set(theme_economist(base_size = 9, base_family = "Proxima Nova", dkpanel = TRUE) +
  ...
  ggsave(outcomes_plot, filename = "outcomes.jpeg", dpi = 200, width = 4.5, height = 4, type = "cairo",  units = "in")
}

# simple model that just uses admits and denies so far to extrapolate to remaining decisions
naive_admit_estimation <- function(admissions_attempts = 20, admits = 4, denies = 6,begin_date = as.Date("2018/01/20"),
  ...
  if ((elapsed_span/timespan - (admits+denies)/admissions_attempts) > 0) {
    print("Hmm, you ought to have heard back from more places")
  } else {
    print("Decisions pace is on track...")
  }
  # future admits and denies tracking
  future_admits <- floor((admissions_attempts - (admits+denies)) * admits/(admits+denies))
